...
const cleanup = async () => {
  ...
  console.log(`🗑️  Deleted old rooms: ${deletedRooms.deletedCount}`);
  ...
  console.log(`🗑️  Deleted old hotels: ${deletedHotels.deletedCount}`);

  // 4. احذف كل الـ Destinations القديمة (اللي معندهاش slug)
  const deletedDests = await Destination.deleteMany({ slug: { $exists: false } });
  console.log(`🗑️  Deleted old destinations: ${deletedDests.deletedCount}`);
